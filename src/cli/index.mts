...
import { DEFAULT_MODE, normalizeMode, RUNTIME_MODES, VALID_MODES, type SeniorModeMode } from "../core/modes.mjs";
...
function ensureWritableJson(filePath: string, data: unknown, force: boolean): boolean {
  if (fs.existsSync(filePath) && !force) return false;
  fs.writeFileSync(filePath, `${JSON.stringify(data, null, 2)}\n`, "utf8");
  return true;
}

function initProject(force: boolean): void {
  ...
  if (!fs.existsSync(statePath) || force) fs.writeFileSync(statePath, `${DEFAULT_MODE}\n`, "utf8");
...
function setMode(mode: SeniorModeMode): void {
...
function printInstructions(modeValue: string | undefined): void {
  const mode = normalizeMode(modeValue || readMode(loadConfig().config) || DEFAULT_MODE);
  if (!mode) fail(`Invalid mode. Expected one of: ${VALID_MODES.join(", ")}`);
  info(getSeniorModeInstructions(mode));
}

function copyDirectory(source: string, destination: string, force: boolean): "copied" | "skipped" {
...
function installCodexSkills(options: string[]): void {
  ...
  info(`Copied: ${copied}`);
  if (skipped) info(`Skipped existing skills: ${skipped} (use --force to overwrite)`);
...
switch (command) {
  ...
  case "-h":
    ...
    break;
  default:
    if ((RUNTIME_MODES as readonly string[]).includes(command)) {
      setMode(command as SeniorModeMode);
    } else {
      warn(usage());
      process.exitCode = 1;
    }
