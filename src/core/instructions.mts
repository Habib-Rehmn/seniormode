...
import { DEFAULT_MODE, isTaskMode, normalizeMode, normalizeRuntimeMode, type SeniorModeMode } from "./modes.mjs";
...
export function filterSkillBodyForMode(body: string, mode: SeniorModeMode): string {
  ...
  return withoutFrontmatter
    ...
    .filter((line) => {
      ...
      if (tableLabel) {
        ...
        if (labelMode) return labelMode === runtimeMode;
      }
...
export function getFallbackInstructions(mode: SeniorModeMode = DEFAULT_MODE): string {
...
export function getSeniorModeInstructions(mode: SeniorModeMode = DEFAULT_MODE): string {
  ...
  if (!base) return getFallbackInstructions(normalized);
